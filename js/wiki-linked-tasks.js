...
(function (global) {
  ...
  var NOTE = "Pra ver as tasks, aperta Ctrl L no jogo\n\nCompletando uma task, libera a próxima\n\nMais detalhes: https://discord.com/channels/1159230367803392022/1159230368940032093/1495879078094311494";
  ...
  function _step(t, n, comboMap) {
    ...
    if (combo) {
      ...
      comboTag = '<span class="lt-combo-tag" title="Mesma espécie em ' + sys.join(' e ') + ' — combe tudo de uma vez" ' +
        'onclick="event.stopPropagation();LinkedTasks._search(\'' + _esc(t.name) + '\')">⚡ Comba com ' + sys.join(' + ') + '</span>';
    }
    var huntBtn = t.hunt
  ...
  function _comboSection() {
    ...
    var cards = combos.map(function (c) {
      ...
      return '<div class="lt-combo-card" onclick="LinkedTasks._search(\'' + _esc(c.display) + '\')" title="Filtrar a Linked por ' + _esc(c.display) + '">' +
  ...
  function _injectCSS() {
    ...
    s.textContent = [
      ...
      '.lt-hunt-btn{margin-top:auto;display:inline-block;text-align:center;padding:7px;border-radius:9px;border:1px solid rgba(96,192,255,.25);background:rgba(96,192,255,.08);color:#60c0ff;font-size:.76rem;font-weight:700;letter-spacing:.4px;text-transform:uppercase;text-decoration:none;transition:all .14s}',
