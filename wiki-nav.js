/* ================================================================
   wiki-nav.js  — versão definitiva com Medal System integrado
   Carregue APÓS app.js e ANTES de wiki-medals.js no index.html:
...
.wn-content-hero-title {
  ...
  line-height: 1;
  background: linear-gradient(135deg,
    #a0c8ff 0%, #ffffff 30%, #c8a0ff 60%, #7ab8ff 100%
  );
...
.wn-card {
  ...
  transition:
    ...
    box-shadow 0.2s,
    background 0.2s;
  animation: wnCardIn 0.3s cubic-bezier(0.16,1,0.3,1) both;
}
.wn-card::before {
  content: '';
  position: absolute;
...
.wn-card:hover {
  ...
  box-shadow: 0 0 24px rgba(var(--wn-rgb, 58,140,255), 0.14), 0 10px 30px rgba(0,0,0,0.5);
...
.wn-breadcrumb-sep { color: rgba(255,255,255,0.12); }
...
@media (max-width: 640px) {
  .wn-grid { grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: 10px; }
  .wn-card { padding: 18px 12px 15px; gap: 8px; border-radius: 14px; }
...
const RENDERERS = {
  ...
  medals:      () => { if (typeof renderMedals      === 'function') renderMedals(); },
  up150:       () => { if (typeof renderUp150       === 'function') renderUp150(); },
  minimap:     () => { if (typeof renderMinimap     === 'function') renderMinimap(); },
};
...
function buildShell() {
  ...
  if (!tabWiki) return;
  /* Guard: nunca cria dois shells */
  if (document.getElementById('wn-shell')) return;
...
function init() {
  ...
  if (window._wnInitDone) return;
  window._wnInitDone = true;
